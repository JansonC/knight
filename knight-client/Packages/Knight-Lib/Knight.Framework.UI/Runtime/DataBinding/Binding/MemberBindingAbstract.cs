...
namespace UnityEngine.UI
{
    public partial class MemberBindingAbstract : MonoBehaviour
    {
        ...
        [Dropdown("ViewPaths")] [HideIf("IsHideViewPath")]
        public string ViewPath;

        [HideIf("IsDisplayConverter")] public bool IsDataConvert;

        [Dropdown("ModelConvertMethods")] [HideIf("IsNeedDataConverter")]
        public string DataConverterMethodPath;

        [Dropdown("ModelPaths")] public string ViewModelPath;
    ...
    {
        [HideInInspector] public string[] ViewPaths = new string[0];
        [HideInInspector] public string[] ModelPaths = new string[0];
        [HideInInspector] public string[] ModelConvertMethods = new string[0];
            ...
            if (this.ViewProp != null)
            {
                if (!this.IsDataConvert)
                {
                    var rViewModelProps = new List<BindableMember<PropertyInfo>>(
                        DataBindingTypeResolve.GetViewModelProperties(this.gameObject,
                            this.ViewProp.Property.PropertyType, this.IsListTemplate));
                ...
                else
                {
                    var rViewModelMethods = new List<BindableMember<MethodInfo>>(
                        DataBindingTypeResolve.GetViewModelConvertMethods(this.ViewProp.Property.PropertyType,
                            this.IsListTemplate));
                    this.ModelConvertMethods =
                        DataBindingTypeResolve.GetAllConvertMethodPaths(rViewModelMethods).ToArray();
                    ...
                    if (!string.IsNullOrEmpty(this.DataConverterMethodPath))
                    {
                        ...
                        if (nIndex >= 0)
                        {
                            ...
                            var rViewModelProps = new List<BindableMember<PropertyInfo>>(
                                DataBindingTypeResolve.GetViewModelProperties(this.gameObject, rParamType,
                                    rViewModelType, this.IsListTemplate));
                            this.ModelPaths = DataBindingTypeResolve.GetAllViewModelPaths(rViewModelProps).ToArray();
                        }
                    }
                }
        ...
        {
            return !IsDataConvert;
        }

        protected bool IsDisplayConverter()
        {
            return GetType() != typeof(MemberBindingOneWay);
        }
    }
}
